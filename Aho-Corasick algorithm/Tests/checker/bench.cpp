...
int main() {
    ...
    auto tm0 = std::chrono::steady_clock::now();//замеряется время лишь на поиск

    std::vector<int> preRes = search(&mainTrie, inpword);

    auto tm1 = std::chrono::steady_clock::now();
    auto delt = tm1 - tm0;
    std::cerr << "search complete| " << std::chrono::duration_cast<std::chrono::milliseconds>(delt).count()<< "ms" <<std::endl;
